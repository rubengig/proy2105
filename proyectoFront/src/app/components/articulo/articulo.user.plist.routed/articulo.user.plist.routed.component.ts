...
@Component({
  selector: 'app-articulo.user.plist.routed',
  templateUrl: './articulo.user.plist.routed.component.html',
  styleUrls: ['./articulo.user.plist.routed.component.css'],
  standalone: true,
  imports: [CommonModule, FormsModule, TrimPipe, RouterModule, SharedHeaderUnroutedComponent],
})
export class ArticuloUserPlistRoutedComponent implements OnInit {
...
getPage() {
  ...
  view(oArticulo: IArticulo) {
    //navegar a la página de edición
    this.oRouter.navigate(['user/articulo/view', oArticulo.id]);
  }
